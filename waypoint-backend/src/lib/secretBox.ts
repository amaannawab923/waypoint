 ...
 * process has to keep (today: a hosted member's own Jira API token — see
 * services/memberCredentials.service.ts).
 *
 * AT12 (ROAD-147): recovered from commit 37c0eb7's parent, where this
 * module previously backed a single, global backend-owned Jira connection
 * that was deleted as redundant once every Jira read started borrowing
 * the desktop's own credential per request. That commit's own message
 * flagged this file as "recoverable... if the follow-up genuinely wants
 * it" for "the deferred backend-auth work" — this is that follow-up: a
 * hosted member's Jira credential now needs a home that isn't a single
 * machine's local file, since the member and the machine holding
 * jira-auth.json aren't guaranteed to be the same thing on the hosted
 * path. seal()/open() themselves are unchanged and were always
 * provider-agnostic; only the caller and its choice of AAD context
 * (a member id now, not a fixed 'jira' string) are new.
 *
