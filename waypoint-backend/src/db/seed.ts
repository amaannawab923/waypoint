  ...
  await tx.insert(schema.proposals).values([
    {
      ...
      snapshot: { evidence: 'src/settings/loader.ts:88', why: 'Not a duplicate of anything open. The loader fetches one row per project inside a map.', agentName: 'Triage Agent' },
      status: 'proposed',
    ...
    },
    {
      ...
      snapshot: { evidence: 'src/export/download.ts:42', why: 'Same export code path. Linking rather than opening a second ticket.', agentName: 'Triage Agent', kindDisplay: 'Link duplicate' },
      status: 'proposed',
    ...
    },
    {
      ...
      snapshot: { fromStateId: CW_STATE_PROGRESS, evidence: 'src/webhooks/retry.ts:120', why: 'Backoff and the 3-attempt cap are implemented and tested; the Stripe-client touch needs a human.', agentName: 'Code Reviewer' },
      status: 'proposed',
    ...
    },
    {
      ...
      snapshot: { why: 'Reassigning to Maya P. now that a human is needed for the Stripe-client touch.', agentName: 'Code Reviewer' },
      status: 'proposed',
      expiresAt: hoursFromNow(23),
      createdAt: hoursAgo(1),
    },
    {
      ...
      snapshot: { evidence: 'src/auth/session.ts:210', why: 'Disclosure prefix is added server-side; the comment will be marked as agent-authored.', agentName: 'Code Reviewer' },
      status: 'proposed',
    ...
    },
    {
      ...
      snapshot: { why: 'Flagged from a request; no repo is linked to this project, so this is text-only reasoning.', agentName: 'Triage Agent' },
      status: 'proposed',
    ...
    // CW-140 above, matching this proposal's real-world effect.
    {
      ...
      snapshot: { why: 'Matched the saved view it watches.', agentName: 'Triage Agent' },
      status: 'executed',
      decidedBy: 'user',
    ...
    // seeded for real below so the ticket and the proposal agree.
    {
      ...
      snapshot: { why: 'Approved by you.', agentName: 'Code Reviewer' },
      status: 'executed',
