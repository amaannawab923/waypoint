...
import { dismissJiraTombstone, listMyJiraTickets, resolveJiraConflict } from '@/data/jiraApi';
...
function FilterChip({
  ...
  return (
    ...
    >
      {swatch && <span className="size-2 shrink-0 rounded-sm" style={{ background: swatch }} />}
      {children}
    </button>
  );
}
...
function LiveSyncIndicator({ lastSyncAt }: { lastSyncAt: string }) {
  ...
  const secs = Math.max(0, Math.round((Date.now() - new Date(lastSyncAt).getTime()) / 1000));
...
export default function MyJiraPage() {
  ...
  const { data: fetchedTickets, loading } = useAsync(() => listMyJiraTickets(), []);
  ...
  async function handleResolveConflict(ticketId: string) {
    try {
      ...
      showErrorToast(err instanceof Error ? err.message : 'Could not re-read this ticket from Jira.');
    }
  }
  ...
  async function handleDismissTombstone(ticketId: string) {
    try {
      ...
      showErrorToast(err instanceof Error ? err.message : 'Could not clear this ticket.');
    }
  }

  const projectCounts = useMemo(() => {
    const counts = new Map<JiraProjectKey, number>();
    for (const t of tickets) counts.set(t.projectKey, (counts.get(t.projectKey) ?? 0) + 1);
    return counts;
  }, [tickets]);
  ...
  const filtered = useMemo(
    () =>
      tickets.filter(
        (t) => (projFilter === 'all' || t.projectKey === projFilter) && (roleFilter === 'all' || t.role === roleFilter),
      ),
  ...
  return (
    <div className="mx-auto max-w-6xl p-6 md:p-8">
      <div className="flex items-center gap-2.5">
        ...
        <h1 className="font-display text-[19px] font-semibold text-text">My Jira</h1>
        {connection && (
      ...
      <p className="mt-1.5 ml-[41px] max-w-[70ch] text-[12.5px] text-text-secondary">
        Everything assigned to you, reported by you, or watched by you — across <b>every</b> Jira project you can
        see, not one board.
        <span className="mt-1 block font-mono text-[11px] text-text-muted">
          assignee = currentUser() OR reporter = currentUser() OR watcher = currentUser() — AND resolution =
          Unresolved
        </span>
      </p>
          ...
          {loading && !fetchedTickets ? (
            ...
            <>
              <div className="mb-2.5 flex flex-wrap items-center gap-1.5">
                <FilterChip active={projFilter === 'all'} onClick={() => setProjFilter('all')}>
                  All {tickets.length}
                </FilterChip>
                ...
                {ROLE_FILTERS.map((r) => (
                  <FilterChip key={r.key} active={roleFilter === r.key} onClick={() => setRoleFilter(r.key)}>
                    {r.label}
                  </FilterChip>
                ))}
              </div>
              ...
              <div className="mb-1.5 flex items-center justify-between gap-2.5 text-[11.5px] text-text-muted">
                <span>
                  {filtered.length} issue{filtered.length === 1 ? '' : 's'} · {visibleProjectCount} Jira project
                  {visibleProjectCount === 1 ? '' : 's'}
                </span>
                {connection && <span>one API call · polls every {connection.pollIntervalSec}s</span>}
              </div>
              ...
              <div className="mt-3 flex items-start gap-2 rounded-[var(--radius-sm)] border border-jira/30 bg-jira-bg px-3 py-2.5 text-[12.5px] text-jira">
                <span>Your own clicks write straight to Jira — no approval step, ~400ms.</span>
              </div>
            </>
          )}
        </div>
      ...
      {drawerTicket && <JiraTicketDrawer ticket={drawerTicket} onClose={() => setDrawerTicketId(null)} />}
    </div>
  );
}
