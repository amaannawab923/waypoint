    ...
    <div className="grid grid-cols-2 gap-2.5">
      ...
      >
        <div
          className={clsx(
            ...
            chosen === 'independent' ? 'bg-accent text-on-accent' : 'bg-surface-3 text-text-secondary',
          )}
        >
          <IconFolder size={14} />
        </div>
        <b className="block text-[13.5px] font-semibold text-text">Independent project</b>
        <p className="mt-0.5 text-xs leading-snug text-text-secondary">
          Tickets, sprints, docs and workstreams — all owned by Waypoint, all local.
        </p>
      </button>
      <button
        type="button"
      ...
      >
        <div
          className={clsx(
            'mb-2 flex size-7 items-center justify-center rounded-md',
            chosen === 'companion' ? 'bg-accent text-on-accent' : 'bg-surface-3 text-text-secondary',
          )}
        >
          <IconRefresh size={14} />
        </div>
        <b className="block text-[13.5px] font-semibold text-text">Companion project</b>
        <p className="mt-0.5 text-xs leading-snug text-text-secondary">
          Your own work from an external tracker, mirrored live — and writable from here.
        </p>
      </button>
    </div>
  );
...
function ProviderStep({ chosen, onPick }: { chosen: 'jira' | null; onPick: (provider: 'jira') => void }) {
  return (
    <div className="flex flex-col gap-2">
      <button
        ...
        className={clsx(
          ...
          chosen === 'jira' ? 'border-accent bg-accent-soft-bg' : 'border-border-strong bg-surface hover:bg-surface-2',
        )}
      >
      ...
      {(['Linear', 'Shortcut'] as const).map((name) => (
        ...
        >
          ...
          <Badge tone="neutral" className="ml-auto px-1.5 py-0 text-[10px] leading-4">
            Not built yet
          </Badge>
        </div>
      ))}
  ...
  if (connectionStatus?.connected) {
    return (
      <div className="flex items-center gap-3 rounded-[var(--radius)] border border-border px-4 py-3.5">
        ...
        <div>
          <b className="block text-[13.5px] font-semibold text-text">{connectionStatus.accountName}</b>
          <div className="text-xs text-text-muted">{connectionStatus.accountEmail}</div>
        </div>
  ...
  return (
    <div>
      <p className="mb-4 text-[13px] leading-relaxed text-text-secondary">
        Waypoint mirrors <b className="text-text">your</b> Jira work — everything assigned to, reported by, or
        watched by you. There&apos;s no board to pick.
      </p>
      <Button variant="primary" className="w-full justify-center" disabled={connecting} onClick={onConnect}>
      ...
      <div className="mt-3.5 flex items-start gap-2 rounded-[var(--radius-sm)] border border-accent/30 bg-accent-soft-bg px-3 py-2.5 text-[12.5px] leading-relaxed text-accent-soft-text">
        Opens Atlassian&apos;s own sign-in in your browser. Waypoint never sees your password, and writes are
...
function SiteStep({ selectedSite, onPick }: { selectedSite: string; onPick: (site: string) => void }) {
  return (
    <div>
      <p className="mb-3 text-[12.5px] text-text-secondary">
        Your account can see two Atlassian sites. Pick the one you work in — this is the only thing left to choose.
      </p>
          ...
          >
            <span
              className={clsx(
                ...
                selectedSite === s.site ? 'border-accent' : 'border-border-strong',
              )}
            >
              {selectedSite === s.site && <span className="absolute inset-[2px] rounded-full bg-accent" />}
            </span>
            <span className="flex-1 text-[13px] font-medium text-text">{s.site}</span>
...
function ConfirmStep({ site, connectionStatus }: { site: string; connectionStatus: JiraConnectionStatus | null }) {
  return (
    <div>
      <div className="mb-3 flex items-center gap-3 rounded-[var(--radius)] border border-border bg-surface-2 p-3.5">
        ...
        <div>
          ...
          <div className="mt-0.5 text-xs text-text-muted">{site} · your work across every project you can see</div>
        </div>
      </div>
      <div className="mb-3 flex gap-5">
        <div>
          <b className="block font-mono text-[15px] font-bold text-text">{connectionStatus?.issueCount ?? 0}</b>
          <span className="text-[11.5px] text-text-muted">issues, {connectionStatus?.projectCount ?? 0} projects</span>
        </div>
        <div>
          <b className="block font-mono text-[15px] font-bold text-text">1</b>
          <span className="text-[11.5px] text-text-muted">API call to load</span>
        </div>
        <div>
          <b className="block font-mono text-[15px] font-bold text-text">{connectionStatus?.pollIntervalSec ?? 15}s</b>
          <span className="text-[11.5px] text-text-muted">refresh, feels live</span>
        </div>
      </div>
      <div className="flex items-start gap-2 rounded-[var(--radius-sm)] border border-jira/30 bg-jira-bg px-3 py-2.5 text-[12.5px] leading-relaxed text-jira">
        Moving, commenting and closing from Waypoint changes the real issues. Sprints, Docs and Workstreams
        don&apos;t appear here — Jira owns those, and Waypoint won&apos;t fake a mirror of them.
      </div>
    </div>
  );
}
...
export function AddProjectWizard({
  ...
  async function handleConnectJira() {
    ...
    try {
      ...
      showErrorToast(err instanceof Error ? err.message : 'Could not connect to Jira.');
    } finally {
      setConnecting(false);
    }
  }
  ...
  async function handleFinishCompanion() {
    ...
    try {
      ...
      showErrorToast(err instanceof Error ? err.message : 'Could not finish connecting Jira.');
    } finally {
      setFinishing(false);
    }
  }
      ...
      footer={
        <>
          ...
          <Button variant="primary" disabled={nextDisabled} onClick={handleNext}>
            {step === TOTAL_STEPS ? (finishing ? 'Creating…' : 'Create project') : 'Continue'}
          </Button>
        </>
      }
    >
      {step === 1 && <TypeStep chosen={chosenType} onPick={setChosenType} />}
      {step === 2 && <ProviderStep chosen={chosenProvider} onPick={setChosenProvider} />}
      {step === 3 && (
        <ConnectStep connecting={connecting} connectionStatus={connectionStatus} onConnect={handleConnectJira} />
      )}
      {step === 4 && <SiteStep selectedSite={selectedSite} onPick={setSelectedSite} />}
      {step === 5 && <ConfirmStep site={selectedSite} connectionStatus={connectionStatus} />}
    </Modal>
  );
}
