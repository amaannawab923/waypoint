...
import { installEngine, verifyInstalledEngine, type EnginePin } from './installer';
...
function makeFixture(
  ...
  if (!opts.omitLauncher) {
    writeFileSync(path.join(root, 'bin', 'emdash-workspace-server'), '#!/bin/sh\necho fake\n');
  }
...
describe('installEngine', () => {
  it('extracts a pinned archive and vouches for the launcher and manifest', async () => {
    ...
    const result = await installEngine(f.paths, { bundledArchiveDir: f.bundledArchiveDir, ...DARWIN_ARM }, f.pin);
  ...
  it('is idempotent: a valid install is left alone, not re-extracted', async () => {
    ...
    await installEngine(f.paths, { bundledArchiveDir: f.bundledArchiveDir, ...DARWIN_ARM }, f.pin);
    // Mark the installed launcher; a re-extract would overwrite the mark.
    writeFileSync(f.paths.launcherPath, '#!/bin/sh\necho marked\n');
    const again = await installEngine(f.paths, { bundledArchiveDir: f.bundledArchiveDir, ...DARWIN_ARM }, f.pin);
  ...
  it('refuses an archive whose sha256 is not the pinned one, and extracts nothing', async () => {
    ...
    const result = await installEngine(f.paths, { bundledArchiveDir: f.bundledArchiveDir, ...DARWIN_ARM }, wrongPin);
    expect(result).toMatchObject({ ok: false, reason: 'sha256-mismatch' });
    await expect(verifyInstalledEngine(f.paths, wrongPin)).resolves.toMatchObject({ ok: false });
  });

  it('refuses a manifest that disagrees with the pin, naming the field', async () => {
    const f = makeFixture('badmanifest', { ...GOOD_MANIFEST, protocolVersion: '2.0.0' });
    const result = await installEngine(f.paths, { bundledArchiveDir: f.bundledArchiveDir, ...DARWIN_ARM }, f.pin);
    expect(result).toMatchObject({ ok: false, reason: 'manifest-mismatch' });
    if (!result.ok) expect(result.message).toMatch(/protocolVersion is 2\.0\.0/);
  });
  ...
  it('refuses to install on a platform the pin was not built for, before touching disk', async () => {
    ...
    const result = await installEngine(
      f.paths,
      { bundledArchiveDir: f.bundledArchiveDir, platform: 'darwin', arch: 'x64' },
      f.pin,
    );
...
describe('ENGINE_PIN and engine.lock.json agree', () => {
  it('names the same version, protocol, target and sha256', () => {
    const lock = JSON.parse(
      readFileSync(path.join(__dirname, '..', '..', '..', 'engine.lock.json'), 'utf8'),
    );
