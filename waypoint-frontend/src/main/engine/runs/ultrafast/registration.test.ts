...
const provisionPythonEnvMock = jest.fn<Promise<{ ok: boolean; message?: string; alreadyProvisioned?: boolean }>, [unknown]>();
jest.mock('./pythonEnv', () => {
...
const running = (since: number): EngineStatus =>
  ({ kind: 'running', since, health: {}, agreed: {}, transport: 'socket' }) as unknown as EngineStatus;
...
beforeEach(() => {
  ...
  provisionPythonEnvMock.mockResolvedValue({ ok: true, alreadyProvisioned: true });

  tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ultrafast-registration-test-'));
  appPath = path.join(tmpRoot, 'app');
  resourcesPath = path.join(tmpRoot, 'resources');
  userData = path.join(tmpRoot, 'userData');
  fs.mkdirSync(path.join(resourcesPath, 'scripts', 'ultrafast'), { recursive: true });
  fs.writeFileSync(path.join(resourcesPath, 'scripts', 'ultrafast-mcp.js'), '// fake');
  fs.writeFileSync(path.join(resourcesPath, 'scripts', 'ultrafast', 'runner.py'), '# fake');
});
...
describe('registerUltrafastBrowser', () => {
  it('registers once a key is configured, uv is available, and the scripts exist', async () => {
    ...
    registerUltrafastBrowser({ supervisor, appPath, resourcesPath, userData, execPath: '/bin/waypoint', logger });
    await flush();
    ...
    expect(server.args).toEqual([path.join(resourcesPath, 'scripts', 'ultrafast-mcp.js')]);
    expect(server.env.ELECTRON_RUN_AS_NODE).toBe('1');
    expect(server.env.ULTRAFAST_TYPESAFE_API_KEY).toBe('ts_live_key');
    expect(server.env.ULTRAFAST_RUNNER_PATH).toBe(path.join(resourcesPath, 'scripts', 'ultrafast', 'runner.py'));
  ...
  it('warns and registers nothing when uv is unavailable', async () => {
    ...
    await flush();
    expect(saveMcpServer).not.toHaveBeenCalled();
    expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('uv is not available'));
  });

  it('provisions in the background when not yet provisioned, then registers', async () => {
    isProvisionedMock.mockReturnValue(false);
    provisionPythonEnvMock.mockResolvedValue({ ok: true, alreadyProvisioned: false });
    const supervisor = fakeSupervisor(running(1));
    registerUltrafastBrowser({ supervisor, appPath, resourcesPath, userData, execPath: '/bin/waypoint', logger });
    await flush();
  ...
  it('does not register when provisioning fails, and retries on the next connection', async () => {
    isProvisionedMock.mockReturnValue(false);
    provisionPythonEnvMock.mockResolvedValueOnce({ ok: false, message: 'uv venv failed' });
    const supervisor = fakeSupervisor(running(1));
    registerUltrafastBrowser({ supervisor, appPath, resourcesPath, userData, execPath: '/bin/waypoint', logger });
    await flush();
    expect(saveMcpServer).not.toHaveBeenCalled();
    ...
    provisionPythonEnvMock.mockResolvedValueOnce({ ok: true, alreadyProvisioned: false });
  ...
  it('stops listening when unsubscribed', async () => {
    ...
    const off = registerUltrafastBrowser({ supervisor, appPath, resourcesPath, userData, execPath: '/bin/waypoint', logger });
    off();
    supervisor.emit(running(1));
    await flush();
    expect(saveMcpServer).not.toHaveBeenCalled();
...
describe('ultrafastAvailability', () => {
  ...
  it('reports false facts truthfully when nothing is set up', () => {
    ...
    isProvisionedMock.mockReturnValue(false);
    expect(
      ultrafastAvailability({ appPath: '/nowhere', resourcesPath: '/nowhere-resources', userData }),
    ).toEqual({
      keyConfigured: false,
      uvAvailable: false,
      provisioned: false,
